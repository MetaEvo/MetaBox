...
class A2C_Agent(Basic_Agent):
    ...
    def rollout_batch_episode(self, 
                              ...
                              required_info = {}):
        """
        Executes batch rollout episodes in parallelized environments and returns the results.
        ...
        # Returns:
        - dict: A dictionary containing the total return for each environment and additional requested information.
        """
        num_cpus = None
        num_gpus = 0 if self.config.device == 'cpu' else torch.cuda.device_count()
        if 'num_cpus' in compute_resource.keys():
