...
class REINFORCE_Agent(Basic_Agent):
    ...
    def rollout_batch_episode(self,
                              ...
                              required_info={}):
        """
        Executes multiple rollout episodes in parallel environments without training.
        ...
        # Returns:
        - dict: A dictionary containing batch rollout results such as return, cost, and additional requested information.
        """
        num_cpus = None
        num_gpus = 0 if self.config.device == 'cpu' else torch.cuda.device_count()
        if 'num_cpus' in compute_resource.keys():
