...
class QLearning_Agent(Basic_Agent):
    ...
    def rollout_batch_episode(self,
                              ...
                              required_info={}):
        """
        Executes multiple episodes in parallel environments without training and returns the results.
        ...
        # Returns:
        - dict: A dictionary containing evaluation results for batch episodes, including cumulative rewards, environment costs, and metadata.
        """
        num_cpus = None
        num_gpus = 0 if self.config.device == 'cpu' else torch.cuda.device_count()
        if 'num_cpus' in compute_resource.keys():
